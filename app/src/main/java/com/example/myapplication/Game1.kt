...
class Game(con: Context?, at: AttributeSet?) : View(con, at) {
    ...
    //뷰의 크기가 변경될 때 호출
    protected fun onSizeChanged(sw: Int, sh: Int, esw: Int, esh: Int) {
        //부모 클래스의 멤버 변수를 참조한다.
    ...
    protected fun onDraw(canvas: Canvas) {
        ...
        val main: Array<Bitmap?> = arrayOfNulls<Bitmap>(12)
        ...
        for (i in 0..11) {
            ...
            if (i == n) {
                ...
                canvas.drawBitmap(main[i], scrw.toFloat() / 2 + xd, scrh.toFloat() / 2 + yd, null)
            }
        }
